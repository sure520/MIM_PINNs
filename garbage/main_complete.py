...
def parse_args():
    ...
    parser.add_argument('--epochs', type=int, default=3000, help='每个同伦步骤的训练轮数')  # 增加训练轮数
    parser.add_argument('--n_homotopy_steps', type=int, default=10, help='同伦步骤数')  # 增加同伦步骤数
    ...
    parser.add_argument('--max_solutions', type=int, default=3, help='最大求解数量')  # 增加最大求解数量
...
def main():
    ...
    config = get_config()
    # 更新设备配置
    config['device']['device'] = str(device)
    data_gen = DataGenerator(config, device=device)
    ...
    for k in range(args.homotopy_init_k, args.homotopy_init_k + args.max_solutions):
        ...
        # 检查解的有效性
        if residual_error < 1e-2 and boundary_error < 1e-3:  # 放宽精度要求
            # 检查是否为新解
            ...
            if is_new_solution:
                ...
                # 可视化结果
                visualizer.plot_solutions([model], [omega2], [k], args.T, args.v)
                visualizer.plot_loss_curves([loss_history], [k])
                
                # 如果找到足够多的解，提前结束
                if len(all_solutions) >= args.max_solutions:
    ...
    if len(all_solutions) > 0:
        # 加载所有模型
        models = []
        for i, solution_dict in enumerate(all_solutions):
            model = MIMHomPINNFusion(
                width=args.width,
                depth=args.depth,
                model_type=args.model_type,
        ...
        # 可视化所有解
        ks = list(range(args.homotopy_init_k, args.homotopy_init_k + len(all_solutions)))
        visualizer.plot_solutions(models, all_omega2, ks, args.T, args.v, save_name='all_solutions.png')
        
        # 绘制所有损失曲线
        visualizer.plot_loss_curves(all_loss_histories, ks, save_name='all_loss_curves.png')
